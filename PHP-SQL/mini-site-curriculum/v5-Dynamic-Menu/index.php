...
------------------------------------------------*/
//1. GET PAGE
$page = $_GET['page'] ?? 'home'; //var_dump($GLOBALS);
...
if(!empty($site_pages)) {
  ...
  //START LOOP
  foreach($site_pages as $page_key => $page_info) {

    $menu = $page_info['menu'];
    $active = $page_key === $page ? ' active' : '';

    //Add to menu html
    $menu_html .= '<li class="menu-item'.$active.'"><a href="?page='.$page_key.'">'.$menu.'</a></li>';
